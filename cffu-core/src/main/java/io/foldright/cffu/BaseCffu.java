...
public abstract class BaseCffu<T, F extends BaseCffu<T, F>> implements Future<T>, CompletionStage<T> {
    ...
    F create(CompletableFuture<T> cf) {
    ...
    @Contract(pure = true)
    <U> Cffu<U> createCffu(CompletableFuture<U> cf) {
        return new Cffu<>(fac, isMinimalStage, cf);
    }

    <E, U extends Iterable<? extends E>> MCffu<E, U> createMCffu(CompletableFuture<U> cf) {
     ...
     */
    @Contract("_, _ -> this")
    public F unsafeOrTimeout(long timeout, TimeUnit unit) {
        checkMinimalStage();
     ...
     * @param unit    a {@code TimeUnit} determining how to interpret the {@code timeout} parameter
     */
    public F completeOnTimeout(@Nullable T value, long timeout, TimeUnit unit) {
        checkMinimalStage();
     ...
     */
    @Contract("_, _, _ -> this")
    public F unsafeCompleteOnTimeout(@Nullable T value, long timeout, TimeUnit unit) {
        checkMinimalStage();
     ...
     */
    public <X extends Throwable> F catchingCompose(
     ...
     * @see Futures#catchingAsync the equivalent Guava method catchingAsync()
     */
    public <X extends Throwable> F catchingComposeAsync(
            Class<X> exceptionType, Function<? super X, ? extends CompletionStage<T>> fallback) {
     ...
     */
    @Contract("_ -> this")
    public F peek(BiConsumer<? super T, ? super Throwable> action) {
     ...
     */
    @Contract("_ -> this")
    public F peekAsync(BiConsumer<? super T, ? super Throwable> action) {
     ...
     */
    @Contract("_, _ -> this")
    public F peekAsync(BiConsumer<? super T, ? super Throwable> action, Executor executor) {
     ...
     */
    @Contract("_ -> this")
    public F completeExceptionallyAsync(Supplier<? extends Throwable> supplier) {
     ...
     */
    @Contract("_, _ -> this")
    public F completeExceptionallyAsync(Supplier<? extends Throwable> supplier, Executor executor) {
        checkMinimalStage();
        // NOTE: do NOT translate (ad hoc input)executor to screened executor; same as CompletableFuture.completeAsync
    ...
    @Contract(pure = true)
    public F copy() {
        return create(fac, isMinimalStage, CompletableFutureUtils.copy(cf));
    }
