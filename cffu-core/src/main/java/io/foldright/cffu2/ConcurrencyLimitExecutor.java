    ...
    public void execute(Runnable command) {
        ...
        try {
            ...
            if (syncRunnerCount >= maxConcurrency) throw new RejectedExecutionException("Task rejected:"
                    ...
                    + " (i.e. base executor runs task on caller thread, e.g. CallerRunsPolicy/DirectExecutor);"
                    + " cannot accept new task in " + this);
            ...
            // NOTE 1: When the submitted task runs synchronously, `locking` is always set to false;
            //   so if `locking` is still true here, the submitted task will run asynchronously.
            // NOTE 2: For asynchronous runs, increment worker count here; for synchronous runs, worker count
            //   is already incremented inside the submitted task before `executor.execute()` returns.
